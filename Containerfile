...
RUN pkg update && \
    ...
    cmake gmake rust git ca_root_nss && \
    pkg clean -ay && rm -rf /var/cache/pkg/* /var/db/pkg/repos/*
...
# CFLAGS: undefine __SSE2__ so mimalloc skips emmintrin.h (not in FreeBSD-clang base).
# RUSTFLAGS: relocation-model=pic forces global-dynamic TLS so the .node file
# can be dlopen'd by Node.js without "No space for static TLS" error.
RUN CFLAGS="-U__SSE2__ -ftls-model=global-dynamic" \
...
ARG NODE_VERSION
ARG PACKAGES="node${NODE_VERSION} npm-node${NODE_VERSION} openssl ca_root_nss"
